...
Native **SwiftUI** client for Wayfind: trip planning, maps, places, and profiles backed by **Supabase**, with **Sign in with Apple** and **Google Sign-In**.

## Requirements

- **Xcode** (recent release recommended) with Swift 5
- **iOS 17.6** or later (app deployment target)
- Apple Developer team for device builds and capabilities (Sign in with Apple)

## Open the project

```bash
open wayfind.xcodeproj
```

Select the **wayfind** scheme, choose a simulator or device, then run (**⌘R**).

Dependencies resolve via **Swift Package Manager**; `Package.resolved` is committed for reproducible builds.

## Configuration

Backend URLs, Supabase anon key, Google OAuth client IDs, and third-party API keys used for places and imagery live in:

`wayfind/AppConfig.swift`

For a fork or a new environment, replace those values with your own Supabase project, OAuth clients, and API keys. Align Supabase Auth redirect URLs with the app URL scheme (`wayfind://`) as described in the comments in that file.
...
- **SwiftUI** app lifecycle (`WayfindApp`)
- **Supabase Swift** for auth and data (`AuthSessionService`, `SupabaseManager`)
- **Google Sign-In** for Google auth with Supabase
- **MapKit** and related UI for trip maps and place discovery
...
1. **Text Search Pro / Enterprise** (`$32–35 / 1,000`) → replace with `MKLocalSearch`
2. **Place Details Enterprise / Pro / Atmosphere** (`$17–25 / 1,000`) → replace with `MKMapItem` from `MKLocalSearch`
3. **Autocomplete** (`$2.83 / 1,000`) → replace with `MKLocalSearchCompleter`
...
Submit the [MapKit JS Increase Request form](https://developer.apple.com/contact/request/mapkitjs-increase/) from your Apple Developer account. Increases to **100k–500k+ service calls/day** are commonly granted within days for legitimate production traffic. There is no cost — Apple's MapKit JS remains free even at elevated quotas.

> Trigger this when sustained daily usage crosses ~60% of your current cap. Don't wait until you're already getting `429`s.
